...
/*
 * Author: EL_D148L0
 * returns the polygon that marks the outline of an island in a hole specified by the points of terrain locations.
 * this is different from getTerrainPolygon since it does not add a layer of extra space around the passed points.
 *
 * Arguments:
 ...
 *
 * Example:
 * [[_pos1, _pos2], _pos1, 0] call ELD_magicTriangle_scripts_fnc_getIslandPolygon;
 *
 * Public: No
 */


scopename "getIslandPolygon";
...
_polygon pushBack _currentPoint;
...
while {true} do {
	...
	if (_iteratingDir == _currentDir) then {	
		(_polygon apply {[_x # 0, _x # 1]}) breakOut "getIslandPolygon"; // if there's no neighboring points and a full loop has been completed
	};
